...
public class MalumSpiritTransmutationRecipes extends RecipeProvider {
    ...
    protected void buildCraftingRecipes(Consumer<FinishedRecipe> consumer) {
        ...
        for (int i = 0; i < runewoodBlocks.length; i++) {
            ...
            new SpiritTransmutationRecipeBuilder(runewoodBlock, soulwoodBlock)
                    .build(consumer, soulwoodBlock.getRegistryName().getPath());
        }
        new SpiritTransmutationRecipeBuilder(Blocks.COBBLESTONE, Blocks.GRAVEL)
                .build(consumer);

        new SpiritTransmutationRecipeBuilder(Blocks.STONE, Blocks.SAND)
                .build(consumer);

        new SpiritTransmutationRecipeBuilder(Blocks.COBBLED_DEEPSLATE, Blocks.NETHERRACK)
                .build(consumer);

        new SpiritTransmutationRecipeBuilder(Blocks.DEEPSLATE, Blocks.SOUL_SAND)
                .build(consumer);
