...
public class SpiritFocusingPage<T extends EntryScreen<T, ?>> extends BookPage<T> {
    ...
    public static <T extends EntryScreen<T, ?>> SpiritFocusingPage<T> fromInput(Item inputItem) {
        return new SpiritFocusingPage<>(s -> s.doesInputMatch(inputItem.getDefaultInstance()));
    }

    public static <T extends EntryScreen<T, ?>> SpiritFocusingPage<T> fromOutput(Item outputItem) {
