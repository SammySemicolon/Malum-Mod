...
public class TaintedGroundDecorator extends AlterGroundTreeDecorator
{
    public final BlockStateProvider stateProvider;
    
    public TaintedGroundDecorator(BlockStateProvider stateProvider)
    {
        super(stateProvider);
    ...
    protected void transformBlock(IWorldGenerationReader reader, Random random, BlockPos pos)
    {
        for (int i = 2; i >= -3; --i)
        {
            ...
            if (Feature.isDirtAt(reader, blockpos))
            {
                ...
                createFlower(reader, random, blockpos.up(), new Block[]{MalumBlocks.SHORT_TAINTED_GRASS.get(), MalumBlocks.TAINTED_GRASS.get(), MalumBlocks.TALL_TAINTED_GRASS.get()}, 0.5f);
    
                createFlower(reader, random, blockpos.up(), new Block[]{MalumBlocks.TAINTED_LAVENDER.get()}, 0.1f);
                break;
            }
