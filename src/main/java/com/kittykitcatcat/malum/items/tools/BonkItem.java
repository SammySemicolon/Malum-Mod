...
public class BonkItem extends SwordItem
{
    ...
    public boolean hitEntity(ItemStack stack, LivingEntity target, LivingEntity attacker)
    {
        if (attacker instanceof PlayerEntity)
        {
            ...
            {
                spawnBonkParticle(attacker,target);
            }
        }
        return super.hitEntity(stack, target, attacker);
    }
    
    public void spawnBonkParticle(LivingEntity attacker, LivingEntity target)
    {
        ...
        {
            ((ServerWorld) attacker.world).spawnParticle(new BonkParticleData(), target.getPosX() - d0 + MathHelper.nextFloat(MalumMod.random, -0.25f, 0.25f), attacker.getPosYHeight(0.85D), target.getPosZ() - d1  + MathHelper.nextFloat(MalumMod.random, -0.25f, 0.25f), 0, d0, 0.0D, d1, 0.0D);
        }
    }
}